...
DoubleLinkedList* GetNodeData(DoubleLinkedList* pNode)
{
   ...
   {
    printf("\nEnter a number: ");
    scanf("%d",&pNode->number);// what is hapening here 
    return pNode;
   }
}

/* Add a node forward */
void AddNodeForward(DoubleLinkedList** pHead)
{
...
Returns : Nothing
*/
void AddNodeReverse(DoubleLinkedList** pHead)
{
    DoubleLinkedList* pNode =  malloc(sizeof(DoubleLinkedList));
    pNode = GetNodeData(pNode);
    if(pNode)
...
void DeleteNode(const int SearchNumber,DoubleLinkedList** pHead)
{
...
void ReverseNodes(DoubleLinkedList** pHead)
{
